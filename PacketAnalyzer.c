...
void TCPExtract(unsigned char* Buffer, int Size)
{
	...
	fprintf(store,"\n");
        if(found == 0){
            ...
            found = 1;
            fprintf(store,"the above tcp header will be used ************************************\n");
        }
        else if(found == 1){
            int ts,td;
            td = ntohs(tcph->source);
            ts = ntohs(tcph->dest);
            if(ts == sport && td == dport){
                fprintf(store,"found the required header above *********************************\n");
                exit(0);
            }
        }
}
